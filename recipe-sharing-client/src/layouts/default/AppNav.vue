<template>
  <v-card class="side-menu-card">
    <v-navigation-drawer expand-on-hover rail class="side-menu">
      ...
      <v-list>
        <router-link :to="{ path: '/login' }" :replace="true" style="text-decoration: none !important;">
          <v-list-item prepend-icon="mdi-account" :title="returnUsername()"
            :subtitle="authenticatedUser?.email" class="icon-color">
          </v-list-item>
        </router-link>
      </v-list>
